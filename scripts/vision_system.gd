...
func setup() -> void:
	los = LOS.new()
	los.setup(map, margin_s, margin_l, map.vsize, map.hsize, map.n_x, map.n_y)
		...
		if a.team == teams.ally:
			for e in units:
				if e.team == teams.enemy:
					var vis_state := los.is_visible(a.position, e.position, a.vision_comp.height, e.vision_comp.height)
